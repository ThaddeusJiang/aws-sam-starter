...
const UpdateCommentSchema = z
    .object({
        content: z.string().min(1, '评论内容不能为空').max(1000, '评论内容不能超过1000个字符').optional(),
...
const updateItem = async (id: string | undefined, input: UpdateCommentInput): Promise<APIGatewayProxyResult> => {
    if (!id) {
        return {
            statusCode: 400,
            body: JSON.stringify({ message: 'ID is required' }),
